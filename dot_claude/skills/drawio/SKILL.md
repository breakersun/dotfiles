---
name: drawio
description: Generate draw.io diagrams as native .drawio files with optional export to PNG/SVG/PDF or browser URL
...
| WSL2 | `cmd.exe /c start "" "$(wslpath -w)"` |
| Windows | `start ` |

**WSL2 notes:**
- `wslpath -w ` converts a WSL2 path (e.g. `/home/user/diagram.drawio`) to a Windows path (e.g. `C:\Users\...`). This is required because `cmd.exe` cannot resolve `/mnt/c/...` style paths.
...
| Edges not rendering | Edge mxCell is self-closing (no child mxGeometry element) | Every edge must have ` ` as a child element |
